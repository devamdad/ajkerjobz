...
class Admin
{
  ...
  public function find_super_admin($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "SELECT * FROM  admins WHERE status='Super'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $data = $stmt->fetchAll();
        return $data;
  ...
  public function find_moderator_admin($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "SELECT * FROM  admins WHERE status='Moderator'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $data = $stmt->fetchAll();
        return $data;
  ...
  public function find_editor_admin($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "SELECT * FROM  admins WHERE status='Editor'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $data = $stmt->fetchAll();
        return $data;
  ...
  public function find_update_admin($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "SELECT * FROM  admins WHERE user_id='$value'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $data = $stmt->fetchAll();
        return $data;
  ...
  public function activate_admin($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "UPDATE  admins SET token = :t, deleted_at = :da WHERE user_id='$this->activateAdminId'";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(
          't' => 0,
  ...
  public function deactivate_admin($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "UPDATE  admins SET token = :t, deleted_at = :da WHERE user_id='$this->deactivateAdminId'";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(
          't' => 1,
          'da' => $value
  ...
  public function update_email($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "UPDATE  admins SET email = :e, updated_at = :ua WHERE user_id='$value'";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(
          'e' => $this->updateEmail,
          'ua' => $this->updateAt
  ...
  public function update_password($newPass,$newHash,$user)
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "UPDATE  admins SET user_pass = :p, user_hash= :h, updated_at = :ua  WHERE user_id='$user'";
        $stmt = $pdo->prepare($query);
        $stmt->execute(array(
          'p' => $newPass,
          'h' => $newHash,
  ...
  public function check_username($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "SELECT * FROM  admins WHERE user_name='$value'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $data = $stmt->fetchAll();
  ...
  public function check_email($value='')
  {
    try{
        $pdo = new PDO('mysql:host=localhost;dbname='.db_name,db_username,db_password);
        $query = "SELECT * FROM  admins WHERE email='$value'";
        $stmt = $pdo->prepare($query);
        $stmt->execute();
        $data = $stmt->fetchAll();
